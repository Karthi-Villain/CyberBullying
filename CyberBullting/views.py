...
def MonitorPost(request):
    if request.method == 'GET':
       ...
       con =  pymysql.connect(host='localhost',port = 3306,user = 'root', password = 'root', database = 'cyber',charset='utf8')
       with con:
          cur = con.cursor()
...
def Signup(request):
    if request.method == 'POST':
      ...
      db_connection =  pymysql.connect(host='localhost',port = 3306,user = 'root', password = 'root', database = 'cyber',charset='utf8')
...
def PostSent(request):
    if request.method == 'POST' and request.FILES['t2']:
        ...
        filename = fs.save('E:/venkat/2021/May21/Cyber/CyberBullying/static/photo/'+str(myfile), myfile)
